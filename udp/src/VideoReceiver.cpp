...
VideoReceiver::VideoReceiver(VideoImageProvider *provider, QObject *parent)
    ...
    const AVCodec *codec = avcodec_find_decoder(AV_CODE_ID_HEVC);
    ...
    while (m_socket->hasPendingDatagrams()) {
        ...
        if (!m_isCorrupt) {
            ...
            if ((uint32_t)m_buffer.size() >= totalSize) {
                decode(m_buffer);
                m_isCorrupt = true; // 等待下一个 slice 0
            }
        }
    }
}
...
void VideoReceiver::decode(const QByteArray &data) {
    ...
    if (avcodec_send_packet(m_codecCtx, &pkt) < 0) {
        avcodec_flush_buffers(m_codecCtx); // 解码失败清空缓存防止持续花屏
        return;
    }
    ...
    while (avcodec_receive_frame(m_codecCtx, m_frame) >= 0) {
        ...
        if (!m_swsCtx) {
            m_swsCtx = sws_getContext(w, h, m_codecCtx->pix_fmt, w, h, AV_PIX_FMT_RGB32, SWS_FAST_BILINEAR, nullptr, nullptr, nullptr);
            m_rgbBuffer = (uint8_t*)av_malloc(av_image_get_buffer_size(AV_PIX_FMT_RGB32, w, h, 1));
        }
